...
export const addPost = (postData) => async (dispatch) => {
    ...
    dispatch(postStatus());
    try {
        const res = await axios.post('/api/posts/addPost', postData);
        dispatch({
            type: ADD_POST,
...
export const getPosts = () => async (dispatch) => {
    ...
    try {
        const res = await axios.get('/api/posts/allPost');
        dispatch({
            type: GET_POSTS,
            payload: res.data,
        });
...
export const getPost = (id) => async (dispatch) => {
    dispatch(setPostLoading());
    try {
        const res = await axios.get(`/api/posts/handle/${id}`);
        dispatch({
            type: GET_POST,
            payload: res.data,
        });
...
export const availablePost = (availableData, idPost) => (dispatch) => {
    dispatch(setPostLoading());
    axios
        .post(`/api/posts/available/${idPost}`, availableData)
        .then((res) => {
            dispatch({
                type: DELETE_POST,
...
export const searchPost = (search) => async (dispatch) => {
    // dispatch(setPostLoading())
    try {
        const res = await axios.get(`/api/posts/search?q=${search}`);
        dispatch({
            type: SEARCH_POST,
            payload: res.data,
        });
...
// Get recommend
export const recommendPost = () => async (dispatch) => {
    dispatch(setPostLoading());
    try {
